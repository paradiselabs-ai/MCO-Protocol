...
def test_modal_basic(modal_token: str):
    ...
    try:
        ...
        result = client.run_inference(test_prompt, "claude-3-5-sonnet")
...
def test_simple_agent(modal_token: str):
    ...
    try:
        ...
        print(f"Direct result success: {direct_result.get('success', False)}")
        ...
        print(f"MCO result success: {mco_result.get('success', False)}")
...
def main():
    ...
    # Get Modal token
    modal_token = input("\n🔑 Enter Modal token (or 'skip' to skip credit tests): ").strip()
    
    if modal_token.lower() == 'skip':
        print("⏭️ Skipping credit-burning tests")
        print("✅ Basic tests passed - ready for HuggingFace deployment")
        return
    
    if not modal_token:
        print("⚠️ No Modal token provided - skipping credit tests")
        print("✅ Basic tests passed - but won't work in production without token")
        return
